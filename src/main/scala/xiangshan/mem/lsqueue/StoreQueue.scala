...
class StoreQueue(implicit p: Parameters) extends XSModule
  ...
  for (((req, resp), i) <- io.forward.map(_.req).zip(io.forward.map(_.resp)).zipWithIndex) {
    ...
    val differentFlag = deqPtrExt(0).flag =/= req.bits.sqIdx.flag
    val forwardMask = req.bits.sqIdxMask
    ...
    val storeSetHitVec = Mux(lfstEnable,
      WireInit(VecInit((0 until StoreQueueSize).map(j => req.bits.uop.loadWaitBit && uop(j).robIdx === req.bits.uop.waitForRobIdx))),
      WireInit(VecInit((0 until StoreQueueSize).map(j => uop(j).storeSetHit && uop(j).ssid === req.bits.uop.ssid)))
    )
    ...
    XSDebug(p"$i f1 ${Binary(canForward1)} f2 ${Binary(canForward2)} " +
      p"sqIdx ${req.bits.sqIdx} pa ${Hexadecimal(req.bits.paddr)}\n"
    )
    ...
    vaddrModule.io.forwardMdata(i) := req.bits.vaddr
    vaddrModule.io.forwardDataMask(i) := req.bits.mask
    paddrModule.io.forwardMdata(i) := req.bits.paddr
    paddrModule.io.forwardDataMask(i) := req.bits.mask
    ...
    val vpmaskNotEqual = (
      (RegEnable(paddrModule.io.forwardMmask(i).asUInt, req.valid) ^ RegEnable(vaddrModule.io.forwardMmask(i).asUInt, req.valid)) &
      RegNext(needForward) &
      GatedRegNext(addrRealValidVec.asUInt)
    ) =/= 0.U
    val vaddrMatchFailed = vpmaskNotEqual && RegNext(req.valid)
    when (vaddrMatchFailed) {
      XSInfo("vaddrMatchFailed: pc %x pmask %x vmask %x\n",
        RegEnable(req.bits.uop.pc, req.valid),
        RegEnable(needForward & paddrModule.io.forwardMmask(i).asUInt, req.valid),
        RegEnable(needForward & vaddrModule.io.forwardMmask(i).asUInt, req.valid)
      );
    }
    ...
    resp.forwardMaskFast := dataModule.io.forwardMaskFast(i)

    // Forward result will be generated 1 cycle later (load_s2)
    resp.forwardMask := dataModule.io.forwardMask(i)
    resp.forwardData := dataModule.io.forwardData(i)
    ...
    resp.dataInvalidFast := dataInvalidMask.orR
    ...
    // load_s2
    resp.dataInvalid := RegNext(resp.dataInvalidFast)
    // check if vaddr forward mismatched
    resp.matchInvalid := vaddrMatchFailed
    ...
    when (RegEnable(req.bits.uop.loadWaitStrict, req.valid)) {
      resp.addrInvalidSqIdx := RegEnable((req.bits.uop.sqIdx - 1.U), req.valid)
    } .elsewhen (addrInvalidFlag) {
      resp.addrInvalidSqIdx.flag := Mux(!s2_differentFlag || addrInvalidSqIdx >= s2_deqPtrExt.value, s2_deqPtrExt.flag, s2_enqPtrExt.flag)
      resp.addrInvalidSqIdx.value := addrInvalidSqIdx
    } .otherwise {
      // may be store inst has been written to sbuffer already.
      resp.addrInvalidSqIdx := RegEnable(req.bits.uop.sqIdx, req.valid)
    }
    resp.addrInvalid := Mux(RegEnable(req.bits.uop.loadWaitStrict, req.valid), RegNext(hasInvalidAddr), addrInvalidFlag)
    ...
    when (dataInvalidFlag) {
      resp.dataInvalidSqIdx.flag := Mux(!s2_differentFlag || dataInvalidSqIdx >= s2_deqPtrExt.value, s2_deqPtrExt.flag, s2_enqPtrExt.flag)
      resp.dataInvalidSqIdx.value := dataInvalidSqIdx
    } .otherwise {
      // may be store inst has been written to sbuffer already.
      resp.dataInvalidSqIdx := RegEnable(req.bits.uop.sqIdx, req.valid)
    }
  }
