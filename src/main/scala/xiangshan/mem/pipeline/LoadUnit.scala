  ...
  with SdtrigExt
{
  val io = IO(new Bundle() {
    ...
    val misalign_ldout = Valid(new LsPipelineBundle)

    // data path
    val tlb           = new TlbRequestIO(2)
    val pmp           = Flipped(new PMPRespBundle()) // arrive same to tlb now
    val dcache        = new DCacheLoadIO
    val sbuffer       = new LoadForwardIO
   ...
   // val refill        = Flipped(ValidIO(new Refill))
    ...
    val fast_rep_in  = Flipped(Decoupled(new LsPipelineBundle))
    val fast_rep_out = Decoupled(new LsPipelineBundle)
  ...
  val s0_src_valid_vec = WireInit(VecInit(Seq(
    ...
    io.replay.valid && io.replay.bits.forwardTLDchannel,
    io.fast_rep_in.valid,
    io.lsq.uncache.valid,
    io.replay.valid && !io.replay.bits.forwardTLDchannel && !s0_rep_stall,
  ...
  def fromFastReplaySource(src: LsPipelineBundle): FlowSource = {
    val out = WireInit(0.U.asTypeOf(new FlowSource))
    out.mask          := src.mask
    out.uop           := src.uop
    out.try_l2l       := false.B
    ...
    out.rep_carry     := src.replayCarry
    out.mshrid        := src.mshrId
    out.frm_mabuf     := src.isMisalignBuf
    out.isFirstIssue  := false.B
    out.fast_rep      := true.B
    out.ld_rep        := src.isLoadReplay
    out.l2l_fwd       := false.B
    out.prf           := LSUOpType.isPrefetch(src.uop.fuOpType) && !src.isVector
    out.prf_rd        := src.uop.fuOpType === LSUOpType.prefetch_r
    out.prf_wr        := src.uop.fuOpType === LSUOpType.prefetch_w
    out.prf_i         := false.B
    out.sched_idx     := src.schedIdx
    out.isvec         := src.isVector
    out.is128bit      := src.is128bit
    out.uop_unit_stride_fof := src.unitStrideFof
    out.reg_offset    := src.regOffset
    out.vecActive     := src.vecActive
    out.is_first_ele  := src.firstEle
    out.usSecondInv   := src.usSecondInv
    out.mbIndex       := src.mbIdx
  ...
  def fromNormalReplaySource(src: LsPipelineBundle): FlowSource = {
    val out = WireInit(0.U.asTypeOf(new FlowSource))
    out.mask          := Mux(src.isVector, src.mask, genVWmask(src.vaddr, src.uop.fuOpType(1, 0)))
    out.uop           := src.uop
    out.try_l2l       := false.B
...
//    out.isFirstIssue  := src.isFirstIssue
  ...
  s0_out.isFastWakeup  := s0_sel_src.l2l_fwd
  s0_out.mshrId        := s0_sel_src.mshrid
  s0_out.isVector      := s0_sel_src.isvec
  s0_out.is128bit        := s0_sel_src.is128bit
  s0_out.isMisalignBuf    := s0_sel_src.frm_mabuf
  s0_out.unitStrideFof := s0_sel_src.uop_unit_stride_fof
  ...
  s0_out.mbIdx          := s0_sel_src.mbIndex
  ...
  s0_out.forwardTLDchannel := s0_src_select_vec(super_rep_idx)
  ...
  s0_out.schedIdx     := s0_sel_src.sched_idx
  ...
  val s1_in         = Wire(new LsPipelineBundle)
  ...
  val s1_fast_rep_dly_kill = RegEnable(io.fast_rep_in.bits.delayedError, io.fast_rep_in.valid) && s1_in.isFastReplay
  val s1_fast_rep_dly_err =  RegEnable(io.fast_rep_in.bits.delayedError, io.fast_rep_in.valid) && s1_in.isFastReplay
  val s1_l2l_fwd_dly_err  = RegEnable(io.l2l_fwd_in.dly_ld_err, io.l2l_fwd_in.valid) && s1_in.isFastWakeup
  ...
  io.lsq.forward.req.valid     := s1_valid && !(s1_exception || s1_tlb_miss || s1_kill || s1_dly_err || s1_prf)
  io.lsq.forward.req.bits.vaddr     := s1_vaddr
  io.lsq.forward.req.bits.paddr     := s1_paddr_dup_lsu
  io.lsq.forward.req.bits.uop       := s1_in.uop
  io.lsq.forward.req.bits.sqIdx     := s1_in.uop.sqIdx
  io.lsq.forward.req.bits.sqIdxMask := 0.U
  io.lsq.forward.req.bits.mask      := s1_in.mask
  io.lsq.forward.req.bits.pc        := s1_in.uop.pc // FIXME: remove it

  io.sbuffer.req := io.lsq.forward.req

  // st-ld violation query
    // if store unit is 128-bits memory access, need match 128-bit
  private val s1_isMatch128 = io.stld_nuke_query.map(x => (x.bits.matchLine || (s1_in.isVector && s1_in.is128bit)))
  ...
  s1_out.delayedError  := s1_dly_err
  s1_out.causeVec(nuke) := s1_nuke && !s1_sw_prf
  ...
  io.lsq.forward.req.bits.sqIdxMask := s1_sqIdx_mask
  if (EnableLoadToLoadForward) {
    when (s1_try_ptr_chasing) {
      io.lsq.forward.req.bits.sqIdxMask := UIntToMask(io.ldin.bits.uop.sqIdx.value, StoreQueueSize)
    }
  }

  io.forward_mshr.valid  := s1_valid && s1_out.forwardTLDchannel
  ...
  loadTrigger.io.fromLoadStore.isVectorUnitStride    := s1_in.isVector && s1_in.is128bit
  ...
  val s2_in     = Wire(new LsPipelineBundle)
  ...
  val s2_isvec  = RegEnable(s1_out.isVector, false.B, s1_fire)
  ...
  when (!s2_in.delayedError && (s2_prf || s2_in.tlbMiss && !s2_tlb_unrelated_exceps)) {
    s2_exception_vec := 0.U.asTypeOf(s2_exception_vec.cloneType)
  }
  val s2_exception = s2_vecActive &&
                    (s2_trigger_debug_mode || ExceptionNO.selectByFu(s2_exception_vec, LduCfg).asUInt.orR)
  val s2_mis_align = s2_valid && GatedValidRegNext(io.csrCtrl.hd_misalign_ld_enable) && !s2_in.isVector &&
                     s2_exception_vec(loadAddrMisaligned) && !s2_exception_vec(breakPoint) && !s2_trigger_debug_mode
  val (s2_fwd_frm_d_chan, s2_fwd_data_frm_d_chan) = io.tl_d_channel.forward(s1_valid && s1_out.forwardTLDchannel, s1_out.mshrId, s1_out.paddr)
  ...
  val s2_mem_amb       = s2_in.uop.storeSetHit &&
                         io.lsq.forward.resp.addrInvalid && RegNext(io.lsq.forward.req.valid)

  val s2_tlb_miss      = s2_in.tlbMiss
  val s2_fwd_fail      = io.lsq.forward.resp.dataInvalid && RegNext(io.lsq.forward.req.valid)
  ...
  //  4. Data contains.
  private val s2_isMatch128 = io.stld_nuke_query.map(x => (x.bits.matchLine || (s2_in.isVector && s2_in.is128bit)))
  ...
  val s2_nuke          = VecInit((0 until StorePipelineWidth).map(w => {
                        ...
                        })).asUInt.orR && !s2_tlb_miss || s2_in.causeVec(nuke)
  ...
  val s2_dcache_should_resp = !(s2_in.tlbMiss || s2_exception || s2_in.delayedError || s2_mmio || s2_prf)
  ...
  val s2_vp_match_fail = (io.lsq.forward.resp.matchInvalid || io.sbuffer.resp.matchInvalid) && s2_troublem
  val s2_safe_wakeup = !s2_out.needReplay && !s2_mmio && !s2_mis_align && !s2_exception // don't need to replay and is not a mmio and misalign
  ...
  io.lsq.ldld_nuke_query.req.bits.dataValid := Mux(s2_full_fwd || s2_fwd_data_valid, true.B, !s2_dcache_miss)
  ...
  io.lsq.stld_nuke_query.req.bits.dataValid := Mux(s2_full_fwd || s2_fwd_data_valid, true.B, !s2_dcache_miss)

  // merge forward result
  // lsq has higher priority than sbuffer
  val s2_fwd_mask = Wire(Vec((VLEN/8), Bool()))
  val s2_fwd_data = Wire(Vec((VLEN/8), UInt(8.W)))
  s2_full_fwd := ((~s2_fwd_mask.asUInt).asUInt & s2_in.mask) === 0.U && !io.lsq.forward.resp.dataInvalid
  // generate XLEN/8 Muxs
  for (i <- 0 until VLEN / 8) {
    s2_fwd_mask(i) := io.lsq.forward.resp.forwardMask(i) || io.sbuffer.resp.forwardMask(i)
    s2_fwd_data(i) := Mux(io.lsq.forward.resp.forwardMask(i), io.lsq.forward.resp.forwardData(i), io.sbuffer.resp.forwardData(i))
  }

  XSDebug(s2_fire, "[FWD LOAD RESP] pc %x fwd %x(%b) + %x(%b)\n",
    s2_in.uop.pc,
    io.lsq.forward.resp.forwardData.asUInt, io.lsq.forward.resp.forwardMask.asUInt,
    s2_fwd_data.asUInt, s2_fwd_mask.asUInt
  )
  ...
  s2_out.mshrHandled       := s2_cache_handled
  ...
  // * dcache miss
  s2_out.causeVec(memoryAmbiguous) := s2_mem_amb && s2_troublem
  s2_out.causeVec(tlbMiss)         := s2_tlb_miss && s2_troublem
  s2_out.causeVec(forwardFail)     := s2_fwd_fail && s2_troublem
  s2_out.causeVec(dcacheReplay)    := s2_mq_nack && s2_troublem
  s2_out.causeVec(dcacheMiss)      := s2_dcache_miss && s2_troublem
  s2_out.causeVec(bankConflict)    := s2_bank_conflict && s2_troublem
  s2_out.causeVec(wpuPredictFail)  := s2_wpu_pred_fail && s2_troublem
  s2_out.causeVec(rarNack)         := s2_rar_nack && s2_troublem
  s2_out.causeVec(rawNack)         := s2_raw_nack && s2_troublem
  s2_out.causeVec(nuke)            := s2_nuke && s2_troublem
  s2_out.dataInvalidSqIdx := io.lsq.forward.resp.dataInvalidSqIdx
  s2_out.addrInvalidSqIdx := io.lsq.forward.resp.addrInvalidSqIdx
  s2_out.replayCarry       := io.dcache.resp.bits.replayCarry
  s2_out.mshrId         := io.dcache.resp.bits.mshr_id
  s2_out.lastBeat       := s2_in.paddr(log2Up(refillBytes))
  s2_out.tlbId          := io.tlb_hint.id
  s2_out.tlbHandled     := !io.tlb_hint.full
  ...
  s1_fast_uop_valid :=
    ...
    !io.lsq.forward.resp.dataInvalidFast
  io.fast_uop.valid := GatedValidRegNext(s1_fast_uop_valid) && (s2_valid && !s2_out.needReplay && !s2_mmio && !(s2_prf && !s2_hw_prf)) && !s2_isvec && !s2_frm_mabuf
  ...
  val s3_isvec        = RegEnable(s2_out.isVector, false.B, s2_fire)
  val s3_vec_alignedType = RegEnable(s2_out.alignedType, s2_fire)
  val s3_vec_mBIndex     = RegEnable(s2_out.mbIdx, s2_fire)
  val s3_frm_mabuf       = s3_in.isMisalignBuf
  ...
  // forwrad last beat
  val s3_fast_rep_canceled = io.replay.valid && io.replay.bits.forwardTLDchannel || io.misalign_ldin.valid || !io.dcache.req.ready
  ...
  io.misalign_buf.valid := io.lsq.ldin.valid && GatedValidRegNext(io.csrCtrl.hd_misalign_ld_enable) && !io.lsq.ldin.bits.isVector
  io.misalign_buf.bits  := s3_in

  /* <------- DANGEROUS: Don't change sequence here ! -------> */
  io.lsq.ldin.bits.dataWenDup := s3_ld_valid_dup.asBools
  ...
  io.fast_rep_out.bits.delayedError := s3_dly_ld_err

  val s3_vp_match_fail = GatedValidRegNext(io.lsq.forward.resp.matchInvalid || io.sbuffer.resp.matchInvalid) && s3_troublem
  val s3_rep_frm_fetch = s3_vp_match_fail
  val s3_ldld_rep_inst =
      io.lsq.ldld_nuke_query.resp.valid &&
      io.lsq.ldld_nuke_query.resp.bits.replayFromFetch &&
  ...
  when (s3_exception || s3_dly_ld_err || s3_rep_frm_fetch) {
    io.lsq.ldin.bits.causeVec := 0.U.asTypeOf(s3_rep_info.causeVec.cloneType)
  } .otherwise {
    io.lsq.ldin.bits.causeVec := VecInit(s3_sel_rep_cause.asBools)
  }
  ...
  s3_vecout.reg_offset        := s3_in.regOffset
  s3_vecout.vecActive         := s3_vecActive
  s3_vecout.is_first_ele      := s3_in.firstEle
  ...
  val s3_revoke = s3_exception || io.lsq.ldin.bits.needReplay
  ...
  io.feedback_slow.bits.hit              := !s3_rep_info.needReplay || io.lsq.ldin.ready
  ...
  io.fast_rep_out.bits := s3_in
  io.fast_rep_out.bits.delayedError := s3_rep_frm_fetch

  val vecFeedback = s3_valid && s3_fb_no_waiting && s3_rep_info.needReplay && !io.lsq.ldin.ready && s3_isvec
  ...
  io.vecldout.bits.hit := !s3_rep_info.needReplay || io.lsq.ldin.ready
  ...
  if (EnableLoadToLoadForward) {
    io.l2l_fwd_out.valid      := s3_valid && !s3_in.mmio && !s3_rep_info.needReplay
  ...
  io.debug_ls.s3_isReplaySlow := io.lsq.ldin.valid && io.lsq.ldin.bits.needReplay
  io.debug_ls.s3_isReplay := s3_valid && s3_rep_info.needReplay // include fast+slow+rs replay
  ...
  XSPerfAccumulate("s0_lsq_replay_vecissue",       io.replay.fire && io.replay.bits.isVector)
  ...
  XSPerfAccumulate("s0_fast_replay_vecissue",      io.fast_rep_in.fire && io.fast_rep_in.bits.isVector)
  ...
  XSPerfAccumulate("s0_forward_tl_d_channel",      s0_out.forwardTLDchannel)
  ...
  XSPerfAccumulate("s2_forward_req",               s2_fire && s2_in.forwardTLDchannel)
